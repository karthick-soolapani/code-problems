...
# Chapter - Advanced 1, Question 2, Exploring Procs, Lambdas, and Blocks: Definition and Arity
...
# Proc is a built-in Ruby class. You can create objects out of it
# One of the ways to create a proc is using Kernel#proc
# The proc object is not executed until it is called using Proc#call
# Procs have lenient arity rules i.e. it doesn't enforce the number of arguments required
# Proc parameters when not supplied default to nil and any extra arguments supplied are ignored.
# Unlike methods procs never raise an arguement error when argument mismatch
...
# lambda object is an instance of Proc class. Lambda.new raises exception as there is no such class
# lambda can be defined either using Kernel#lambda or literal syntax ->
# Unlike block and proc lambda have strict arity rules.
# It enforces the number of arguments required like a method
...
# blocks can be passed implicitly to all methods even if they are not defined as parameters
# blocks have lenient arity rules i.e. it doesn't enforce the number of arguments required
# yield raises an exception when a block is not supplied. Kernel#block_given? can be used to mitigate this
...
# Arguments passed via yield must be captured as block parameters to be accessible
