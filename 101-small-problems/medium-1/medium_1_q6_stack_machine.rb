...
# Chapter - Medium 1, Question 6, Stack Machine Interpretation
...
def minilang(string)
  ...
  commands.each do |command|
    ...
    when 'ADD', 'SUB', 'MULT', 'DIV', 'MOD'
      register = register.send(operations[command.to_sym], stack.pop)
    else 'Invalid token'
    end
  end
end
...
#1 - USING METHODS FOR EACH COMMAND
def st_print
